...
import MODAL from "./delguur_modal";
...
const Delguur = () => {
  ...
  const [max_id, setmax_id] = useState(0);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //   })
        //   .value();
        var mm = _.orderBy(response.data, ["id"], ["desc"]);
  ...
  return (
    <div className="text-sm p-2">
      ...
      <h1 className="text-lg font-bold  text-red-600 pb-2 ">Дэлгүүр</h1>{" "}
      <DataTable
        size="small"
        value={data}
        dataKey="id"
